...
#         ϕ1 = exp(im * 2π * rand())
#         ϕ2 = exp(im * 2π * rand())
#         for q ∈ axes(prob.p.ϵs[1],3)
#             for f ∈ axes(prob.p.ϵs[1],2)
#                 prob.p.ϵs[1][k,f,q] *= ϕ1
#                 prob.p.ϵs[1][k+3,f,q] *= ϕ2
#             end
#         end
#     end
#     return nothing
...
function reset_prob_diffusion!(prob)
    reset_prob!(prob)
...
function compute_σ(sols)
    xs = [x(sol.u[end]) for sol ∈ sols if survived(sol)]
    ys = [y(sol.u[end]) for sol ∈ sols if survived(sol)]
    zs = [z(sol.u[end]) for sol ∈ sols if survived(sol)]
    σ_x = σ_fit(xs)
    σ_y = σ_fit(ys)
    σ_z = σ_fit(zs)
    σ = (σ_x * σ_y * σ_z)^(1/3)
...
function compute_density(sols)
    N = 4815
    n_survived = sum(1 for sol ∈ sols if survived(sol)) / length(sols)
    σ = compute_σ(sols)
    return N * n_survived / σ^3 / (2π)^(3/2) * 1e-6
end
...
using Bootstrap
function distributed_compute_diffusion(coord_idx, prob, prob_func, n_trajectories, t_end, τ_total, n_times, scan_func, scan_values)
    
    diffusions = zeros(length(scan_values))
    diffusion_errors = zeros(length(scan_values))
    diffusions_over_time = zeros(length(scan_values), n_times)
            ...
            for (i, scan_value) ∈ enumerate(scan_values)
                ...
                for pid ∈ workers()
                    future = @spawnat pid begin
                        scan_func(prob, scan_value)
                        compute_diffusion(coord_idx, prob, prob_func, n_trajectories, t_end, τ_total, n_times, channel)
                    end
                    push!(futures, future)
                end
                rets = fetch.(futures)
                ...
                diffusions_over_time[i,:] = diffusion_over_time
